...
from station.serializers import FacilitySerializer, BusSerializer, BusListSerializer, BusRetrieveSerializer
...
def sample_bus(**params) -> Bus:
    ...
    defalts.update(params)
    return Bus.objects.create(**defalts)


class UnauthenticatedBusAoiTestCase(TestCase):
    def setUp(self):
...
class AuthenticatedBusAoiTestCase(TestCase):
    def setUp(self):
        self.client = APIClient()
